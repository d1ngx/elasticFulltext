...
- 与 AIRAG 共用唯一正文索引和重任务锁，全文提取与向量写入不会并发争抢磁盘。
- 正文增加 `extractVersion`，提取字符上限变化后自动重新核对；AIRAG 只消费版本匹配的新正文。
- 默认正文上限提升到 100 万字符，最大文件默认调整为 30 MB。
- 批量读取 ES 文档并仅更新变动的元数据；删除失败会保留状态供后续重试。
- 使用独立背压实现并共享迟滞状态，配置页显示暂停原因和自动恢复说明。
